...
class Account extends BaseController
{
    function index()
    {
        if ($this->session->active) {
            ...
            $cooperator['department'] = $this->departmentModel->where('department_id',
              $cooperator['cooperator_department_id'])->first();
            $cooperator['location'] = $this->locationModel->where('location_id',
              $cooperator['cooperator_location_id'])->first();
            $cooperator['payroll_group'] = $this->payrollGroupModel->where('pg_id',
    ...
    function change_password()
    {
        ...
        $cooperator_data = [
          ...
          'cooperator_password' => password_hash($password, PASSWORD_DEFAULT)
        ];
        $this->cooperatorModel->save($cooperator_data);
        $response_data['success'] = true;
    ...
    function update_details()
    {
        ...
        if (!$post_data['cooperator_email']) {
            $response_data['success'] = false;
            $response_data['msg'] = 'Email is required';
            return $this->response->setJSON($response_data);
        }

        if (!$post_data['cooperator_telephone']) {
            $response_data['success'] = false;
            $response_data['msg'] = 'Phone is required';
            return $this->response->setJSON($response_data);
        }

        $cooperator_data = [
          'cooperator_id' => $cooperator['cooperator_id'],
          'cooperator_email' => $post_data['cooperator_email'],
          'cooperator_telephone' => $post_data['cooperator_telephone']
        ];
        $this->cooperatorModel->save($cooperator_data);
        $response_data['success'] = true;
        $response_data['msg'] = 'Basic details updated successfully!';
        return $this->response->setJSON($response_data);
    }
